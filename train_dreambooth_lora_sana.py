...
def log_validation(
    ...
    for i, img in enumerate(images):
        save_path = os.path.join(
            save_dir,
            f"epoch{epoch}_{'final' if is_final_validation else 'val'}_{i}.png"
        )
        img.save(save_path)
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        #     self.custom_instance_prompts = None
            ...
            # Collect image files (sorted) and look for matching .txt captions with same stem.
            image_extensions = {".jpg", ".jpeg", ".png", ".webp", ".bmp", ".tiff"}
            img_paths = sorted([p for p in self.instance_data_root.iterdir() if p.suffix.lower() in image_extensions])

            if len(img_paths) == 0:
                raise ValueError(f"No image files found in {instance_data_root}")

            # load images and captions (if present)
            instance_images = []
            captions = []
            for p in img_paths:
                img = Image.open(p)
                ...
                if txt_path.exists():
                    try:
                        ...
                        captions.append(text if text != "" else None)
                    except Exception:
                        captions.append(None)
                else:
                    captions.append(None)

            # If at least one caption found, build custom_instance_prompts repeated per image
            if any(c is not None for c in captions):
                self.custom_instance_prompts = []
                for cap in captions:
                    final_caption = cap if cap is not None else instance_prompt  # fallback per-image
                    self.custom_instance_prompts.extend(itertools.repeat(final_caption, repeats))
            else:
                self.custom_instance_prompts = None
        ...
        if self.custom_instance_prompts is not None:
            print(f"[DEBUG] Using per-image captions. Example: {self.custom_instance_prompts[:3]}")
        else:
            print("[DEBUG] Using only the global instance_prompt for all images.")
